...
{
    protected readonly TContext BaseContext;
    ...
    {
        return BaseContext.Set<TEntity>();
    }
    ...
    public async Task<TEntity> AddAsync(TEntity entity, CancellationToken cancellationToken = default)
    {
        ...
        await BaseContext.AddAsync(entity, cancellationToken);
        try
        {
        await BaseContext.SaveChangesAsync(cancellationToken);
    ...
    )
    {
        foreach (TEntity entity in entities)
            EditEntityPropertiesToAdd(entity);
        await BaseContext.AddRangeAsync(entities, cancellationToken);
        await BaseContext.SaveChangesAsync(cancellationToken);
        return entities;
    }
    ...
    )
    {
        foreach (TEntity entity in entities)
            EditEntityPropertiesToUpdate(entity);
        BaseContext.UpdateRange(entities);
        await BaseContext.SaveChangesAsync(cancellationToken);
        return entities;
    }
    ...
    public async Task<List<TEntity>> GetForListAsync(
    Expression<Func<TEntity, bool>>? predicate = null,
    Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>>? orderBy = null,
    Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>>? include = null,
    bool withDeleted = false,
    ...
    {
        IEnumerable<IForeignKey> foreignKeys = BaseContext.Entry(entity).Metadata.GetForeignKeys();
        IForeignKey? oneToOneForeignKey = foreignKeys.FirstOrDefault(fk =>
            fk.IsUnique && fk.PrincipalKey.Properties.All(pk => BaseContext.Entry(entity).Property(pk.Name).Metadata.IsPrimaryKey())
        );

        if (oneToOneForeignKey != null)
        {
            string relatedEntity = oneToOneForeignKey.PrincipalEntityType.ClrType.Name;
            IReadOnlyList<IProperty> primaryKeyProperties = BaseContext.Entry(entity).Metadata.FindPrimaryKey()!.Properties;
    ...
    )
    {
        ...
        foreach (INavigation? navigation in navigations)
        {
            ...
            if (navigation.IsCollection)
            {
                if (navValue == null)
                {
                    IQueryable query = BaseContext.Entry(entity).Collection(navigation.PropertyInfo.Name).Query();
            ...
            else
            {
                if (navValue == null)
                {
                    IQueryable query = BaseContext.Entry(entity).Reference(navigation.PropertyInfo.Name).Query();
